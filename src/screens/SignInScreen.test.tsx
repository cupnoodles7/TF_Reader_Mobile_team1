...
//   3. Tapping the backdrop dismisses; tapping the sheet body does not.
...
const mockNavigation = { goBack: mockGoBack } as unknown as Nav;
...
describe('SignInScreen', () => {
  ...
  it('tapping the backdrop calls goBack; tapping the sheet body does not', async () => {
    mockIsOnline.mockReturnValue(true);
    useInstitutionStore.setState({ selectedInstitution: IMPERIAL });

    await render(<SignInScreen navigation={mockNavigation} route={{} as any} />);

    // Sheet body tap — should be swallowed
    fireEvent.press(screen.getByText('Imperial College London'));
    expect(mockGoBack).not.toHaveBeenCalled();

    // Backdrop tap — should dismiss
  ...
  it('disables the sign-in button when offline and pressing it does not trigger goBack', async () => {
    ...
    // getByLabelText finds the button by accessibilityLabel regardless of disabled state.
    // The handleSignIn guard (!isOnline) ensures goBack is never called even if
    // fireEvent bypasses the disabled prop.
    fireEvent.press(screen.getByLabelText('Sign in with institution'));
    expect(mockGoBack).not.toHaveBeenCalled();
  });
